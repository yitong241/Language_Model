"""
SwiGLU activation ablation.
Sinusoidal PE | Pre-LayerNorm | SwiGLU MLP | GPT-2 BPE tokenizer
"""
...
class SwiGLU(nn.Module):
    def __init__(self, d, dropout):
        super().__init__()
        hidden_dim = (8 * d) // 3
        self.w1 = nn.Linear(d, hidden_dim)   # value branch
        self.w2 = nn.Linear(d, hidden_dim)   # gate branch
        self.dropout = nn.Dropout(dropout)
        self.w3 = nn.Linear(hidden_dim, d)   # down projection
...
class attention_net(nn.Module):
    ...
    def __init__(self, vocab_size, d, num_heads, num_blocks, seq_length, dropout):
        ...
        for block in self.layer2.decoder.TR_Blocks:
            ...
            nn.init.normal_(block.MLP.w3.weight, mean=0, std=residual_std)
