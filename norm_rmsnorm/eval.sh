#!/bin/bash
#SBATCH --job-name=eval_norm_rmsnorm
...
if [[ "$CHECKPOINT" != /* ]]; then
    CHECKPOINT="norm_rmsnorm/$CHECKPOINT"
fi

echo "Checkpoint: $CHECKPOINT"

if [ -n "$PROMPT" ]; then
    python -u eval.py --model-dir norm_rmsnorm --checkpoint "$CHECKPOINT" --prompt "$PROMPT"
else
    python -u eval.py --model-dir norm_rmsnorm --checkpoint "$CHECKPOINT"
fi
