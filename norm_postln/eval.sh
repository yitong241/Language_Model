#!/bin/bash
#SBATCH --job-name=eval_norm_postln
...
if [[ "$CHECKPOINT" != /* ]]; then
    CHECKPOINT="norm_postln/$CHECKPOINT"
fi

echo "Checkpoint: $CHECKPOINT"

if [ -n "$PROMPT" ]; then
    python -u eval.py --model-dir norm_postln --checkpoint "$CHECKPOINT" --prompt "$PROMPT"
else
    python -u eval.py --model-dir norm_postln --checkpoint "$CHECKPOINT"
fi
