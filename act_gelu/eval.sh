...
if [[ "$CHECKPOINT" != /* ]]; then
    CHECKPOINT="act_gelu/$CHECKPOINT"
fi

echo "Checkpoint: $CHECKPOINT"

if [ -n "$PROMPT" ]; then
    python -u eval.py --model-dir act_gelu --checkpoint "$CHECKPOINT" --prompt "$PROMPT"
else
    python -u eval.py --model-dir act_gelu --checkpoint "$CHECKPOINT"
fi
