...
def main(model_dir, output_dir):
    ...
    # ── Eval function ────────────────────────────────────────────────────────
    @torch.no_grad()
    def eval_on_test_set_pe():
        for TR_block in net.layer2.decoder.TR_Blocks:
            TR_block.MHA.capture_attn = True

        # One list per layer, holding running sum of (heads, seq, seq)
        ...
        for count in range(0, eval_data.size(0) - seq_length, seq_length):
            ...
            for i, block in enumerate(net.layer2.decoder.TR_Blocks):
                attn_weights = block.MHA.attn_weights.float() # [batch_size, num_heads, seq_len, seq_len]
        ...
        # Block sum: [num_heads, seq_len, seq_len], summed over all the data points
        attn_mean = [block_sum / (num_batches * bs) for block_sum in attn_sum] 
        attn_mean = torch.stack(attn_mean) # [TR_BLOCK, num_heads, seq_len, seq_len]
    ...
    for epoch in range(num_epochs):
        ...
        eval_loss, mean_attn_scores = eval_on_test_set_pe()
        ...
        if eval_loss < best_loss:
            ...
            torch.save({
                ...
                'eval_loss': eval_loss,
                'mean_attn_scores': mean_attn_scores,
            }, checkpoint_path)
