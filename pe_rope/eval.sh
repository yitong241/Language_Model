...
if [[ "$CHECKPOINT" != /* ]]; then
    CHECKPOINT="pe_rope/$CHECKPOINT"
fi

echo "Checkpoint: $CHECKPOINT"

if [ -n "$PROMPT" ]; then
    python -u eval.py --model-dir pe_rope --checkpoint "$CHECKPOINT" --prompt "$PROMPT"
else
    python -u eval.py --model-dir pe_rope --checkpoint "$CHECKPOINT"
fi
