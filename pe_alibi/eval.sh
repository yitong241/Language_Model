...
if [[ "$CHECKPOINT" != /* ]]; then
    CHECKPOINT="pe_alibi/$CHECKPOINT"
fi

echo "Checkpoint: $CHECKPOINT"

if [ -n "$PROMPT" ]; then
    python -u eval.py --model-dir pe_alibi --checkpoint "$CHECKPOINT" --prompt "$PROMPT"
else
    python -u eval.py --model-dir pe_alibi --checkpoint "$CHECKPOINT"
fi
